# coding:utf-8
# 导入tensorflow。这句import tensorflow as tf是导入TensorFlow约定俗成的做法
...
# 根据y, y_构造交叉熵损失
# tf.reduce_mean根据给出的axis在input_tensor上求平均值。除非keep_dims为真，axis中的每个的张量秩会减少1。如果keep_dims为真，求平均值的维度的长度都会保持为1.如果不设置axis，所有维度上的元素都会被求平均值，并且只会返回一个只有一个元素的张量。
